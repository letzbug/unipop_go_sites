...
function guideOptions(selected='',tutorialPath='',lieu=null){
  const guides=`<option value="">Aucun guide technique</option>`+(data.guides||[]).map(g=>`<option value="${esc(g.id)}" ${String(g.id)===String(selected)?'selected':''}>${esc(g.title||'Guide sans titre')}</option>`).join('');
  const tutorials=(lieu?.tutorials||[]).filter(t=>t.path||t.url).map(t=>{
    const p=t.path||t.url;
    return `<option value="tutorial:${esc(p)}" ${String(p)===String(tutorialPath)?'selected':''}>Tutoriel — ${esc(t.title||t.name||'Tutoriel')}</option>`;
  }).join('');
  return guides+(tutorials?`<optgroup label="Tutoriels du lieu">${tutorials}</optgroup>`:'');
}
...
async function renderRooms(){
  ...
  editor.innerHTML=`
    ...
    <div class="room-card room-card-editor">
      ...
      <select data-rguide="${roomIndex}">${guideOptions(r.guideId||'',r.tutorialGuidePath||'',x)}</select>
  ...
  $$('[data-rguide]').forEach(e=>e.onchange=()=>{
    const room=x.rooms[+e.dataset.rguide];
    if(String(e.value).startsWith('tutorial:')){
      room.guideId='';
      room.tutorialGuidePath=String(e.value).slice(9);
    }else{
      room.guideId=e.value;
      room.tutorialGuidePath='';
    }
